"Name: Automated Snappy NDVI output"

"Description: The following script ingests a single Sentinel-2 tile and processes the NDVI calculation."

"Minimum Requirements: Modules: Snappy, Numpy, os, sys",

"Inputs: 1 zipped Sentinel-2 folder"

"Outputs: 1 GeoTIFF: 1 clipped site of the Wexford Slobs of calculated NDVI",
...
def subset_dim(Output,dimfile, filename):
    
      # Read the outputted NDVI product from file
      ...
      path_collocated = os.path.join(Output, filename +'.tif')

      # Write the image as a .GeoTIFF
      ProductIO.writeProduct(sub_product, path_collocated, 'GeoTIFF')


def raw_processing(Input,Output,dimfile,filename):
       ...
       # Select bands from data, note B3 + B8 are needed for calculating NDVI
       B4 = product.getBand('B4')
       B8 = product.getBand('B8')

       # Create a new NDVI product and add a new band for NDVI
       ...
       ndviProduct.writeHeader(output_path)

       # Create 2 new areas for calculating the NDVI bands
       b4 = numpy.zeros(width, dtype=numpy.float32)
       b8 = numpy.zeros(width, dtype=numpy.float32)

       # Loop through all pixels and calculate the NDVI for that pixel
       for y in range(height):
           b4 = B4.readPixels(0, y, width, 1, b4)
           b8 = B8.readPixels(0, y, width, 1, b8)
           # Excute the following formula for NDVI
           ndvi = (b8 - b4) / (b8 + b4)
           ndviBand.writePixels(0, y, width, 1, ndvi)
       # Close the product
       ndviProduct.closeIO()

       subset_dim(Output,dimfile, filename)

def main():

    Input_location = r'C:\Users\ckelly.COMPASSINFORMAT\Echoes\Ireland\Sentinel-2\Repo'
    Output = r'D:\Projects\Active\Intereg_Echeos\Database\TimeSeries\NDVI\timeseries_files'
    dimfile = 'snappy_ndvi_output.dim'
    ...
    #    print(filename)
    #    try:
    #        raw_processing(Input,Output,dimfile,filename)
    #    except:
