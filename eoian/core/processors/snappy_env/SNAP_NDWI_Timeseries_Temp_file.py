...
def segmentation_process(Output, filename,sub_product):
    ...
    targetBand.expression = 'if ndwi >= 0.5 AND ndwi <=0.998 theN 1 else 0'
    ...
    path_collocated = os.path.join(Output, filename + '_NDWI_sgement.tif')
...
def raw_processing(Input,Output,dimfile,filename):
       ...
       sub_product = subset_dim(Output,dimfile, filename)
       
       # Specific pixel values can be extracted from the image using a select Query
       segmentation_process(Output, filename,sub_product)

def main():

    Input_location = r'C:\Users\ckelly.COMPASSINFORMAT\Echoes\Ireland\Sentinel-2\Repo'
    Output = r'D:\Projects\Active\Intereg_Echeos\Database\TimeSeries\NDWI\timeseries_files'
    dimfile = 'snappy_ndwi_output.dim'
